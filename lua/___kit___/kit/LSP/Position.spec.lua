...
describe('kit.LSP.Position', function()
  ...
  describe('.cursor', function()
    it('should return utf8 position', function()
      ...
      assert.equal(Position.cursor(LSP.PositionEncodingKind.UTF8).character, utf8)
    end)
    it('should return utf16 position', function()
      vim.api.nvim_buf_set_lines(0, 0, -1, false, { text })
      vim.api.nvim_win_set_cursor(0, { 1, #text })
      assert.equal(Position.cursor(LSP.PositionEncodingKind.UTF16).character, utf16)
    end)
    it('should return utf32 position', function()
      vim.api.nvim_buf_set_lines(0, 0, -1, false, { text })
      vim.api.nvim_win_set_cursor(0, { 1, #text })
      assert.equal(Position.cursor(LSP.PositionEncodingKind.UTF32).character, utf32)
    end)
  end)
