...
describe('kit.System', function()
  ...
  describe('DelimiterBuffering', function()
    ...
    it('should buffering by single character delimiter', function()
      local buffer, consumed
      consumed = {}
      buffer = System.DelimiterBuffering.new({ delimiter = '\0' }):create(function(text)
        table.insert(consumed, text)
      end)
      buffer.write('1')
      buffer.write('\x002')
      buffer.write('3\x00')
      buffer.write('\x004\x00')
      buffer.write('5')
      buffer.write('\x00')
      buffer.write('6')
      buffer.write('\x00')
      buffer.write('\x007')
      buffer.close()
      assert.are.same(consumed, { '1', '23', '', '4', '5', '6', '', '7' })

      consumed = {}
      buffer = System.DelimiterBuffering.new({ delimiter = '\0' }):create(function(text)
        table.insert(consumed, text)
      end)
      buffer.write('1\x002\x003\x004\x005\x006\x00\x007')
      buffer.close()
      assert.are.same(consumed, { '1', '2', '3', '4', '5', '6', '', '7' })
    end)
  end)
